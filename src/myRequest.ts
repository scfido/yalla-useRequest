...
METHODS.forEach(method => {
    //@ts-ignore
    useRequest[method] = (service: any, options: any = {}) => {
        if (typeof service === "string")
            return useRequest({ url: service, method }, options);

        if (typeof service === 'object') {
            return useRequest({ ...service, method }, options);
        }
...
function useRequest(service: any, options: any = {}) {
    ...
    return useUmiRequest(service, {
      /*FRS*/ formatResult: res => res?.data /*FRE*/,
        ...
        onError: (error: RequestError, p) => {
            //1:返回ture：屏蔽后续错误，无错误通知
            //2:返回false：抛出原有错误，有错误通知
            //3:throw Error：抛出原有错误，错误通知显示新的错误
            if (userErrorHandle) {
                try {
    ...
    function showError(error: RequestError) {
